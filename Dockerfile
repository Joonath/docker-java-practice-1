...
# DIRECTORY to
ADD . /usr/local/demo_java_docker
...
RUN cd /usr/local/demo_java_docker && mvn assembly:assembly
...
CMD ["java", "-cp", "/usr/local/demo_java_docker/target/demo_java_docker-1.0.0-alpha-jar-with-dependencies.jar", "id.web.jonathan.App"]